...
const GhmcSecondPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('/ghmc.json');
        if (!response.ok) {
...
checkBoxes();
  ...
  if (error) {
    return <p className="bg-purple-500">{error}</p>;
  }

  return (
    <div>
      <div className="nav">
        <h1 className={`title ${parallaxClass} `} style={{fontFamily:"playfair display",color:"#A855F7"}}>
          {language === 'en' ? 'GHMC Services' : 'GHMC సేవలు'}
        </h1>
      ...
      <div className="container">
        ...
        {selectedService ? (
          <div className="service-details">
            <h1 className="title " style={{color:"#A855F7"}}>
  ...
  <thead >
    <tr >
      <th style={{backgroundColor:"#A855F7"}}>{language === 'en' ? 'S.No' : 'క్రమ సంఖ్య'}</th>
      <th style={{backgroundColor:"#A855F7"}}>{language === 'en' ? 'Service Name' : 'సేవ పేరు'}</th>
      <th style={{backgroundColor:"#A855F7"}}>{language === 'en' ? 'Actions' : 'చర్యలు'}</th>
    </tr>
  </thead>
  <tbody>
