...
const TgspdcSecondPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('/tgspdcl.json');
        if (!response.ok) {
...
checkBoxes();
  ...
  if (error) {
    return <p className="bg-yellow-500">{error}</p>;
  }

  return (
    <div>
      <div className="nav">
        <h1 className={`title ${parallaxClass} `} style={{fontFamily:"playfair display",color:"#EAB308"}}>
          {language === 'en' ? 'TGSPDCL Services' : 'TGSPDCL సేవలు'}
        </h1>
      ...
      <div className="container">
        ...
        {selectedService ? (
          <div className="service-details">
            <h1 className="title " style={{color:"#EAB308"}}>
  ...
  <thead >
    <tr >
      <th style={{backgroundColor:"#EAB308"}}>{language === 'en' ? 'S.No' : 'క్రమ సంఖ్య'}</th>
      <th style={{backgroundColor:"#EAB308"}}>{language === 'en' ? 'Service Name' : 'సేవ పేరు'}</th>
      <th style={{backgroundColor:"#EAB308"}}>{language === 'en' ? 'Actions' : 'చర్యలు'}</th>
    </tr>
  </thead>
  <tbody>
...
export default TgspdcSecondPage;
