...
checkBoxes();
  ...
  if (error) {
    return <p className="bg-teal-500">{error}</p>;
  }

  return (
    <div>
      <div className="nav">
        <h1 className={`title1 ${parallaxClass} `} style={{fontFamily:"playfair display",color:"#14B8A6"}}>
      ...
      <div className="container">
        ...
        {selectedService ? (
          <div className="service-details">
            <h1 className="title " style={{color:"#14B8A6"}}>
  ...
  <thead >
    <tr >
      <th style={{backgroundColor:"#14B8A6"}}>{language === 'en' ? 'S.No' : 'క్రమ సంఖ్య'}</th>
      <th style={{backgroundColor:"#14B8A6"}}>{language === 'en' ? 'Service Name' : 'సేవ పేరు'}</th>
      <th style={{backgroundColor:"#14B8A6"}}>{language === 'en' ? 'Actions' : 'చర్యలు'}</th>
    </tr>
  </thead>
  <tbody>
