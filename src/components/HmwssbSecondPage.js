...
checkBoxes();
  ...
  if (error) {
    return <p className="bg-orange-500">{error}</p>;
  }

  return (
    <div>
      <div className="nav">
        <h1 className={`title ${parallaxClass} `} style={{fontFamily:"playfair display",color:"#F97316"}}>
      ...
      <div className="container">
        ...
        {selectedService ? (
          <div className="service-details">
            <h1 className="title " style={{color:"#F97316"}}>
  ...
  <thead >
    <tr >
      <th style={{backgroundColor:"#F97316"}}>{language === 'en' ? 'S.No' : 'క్రమ సంఖ్య'}</th>
      <th style={{backgroundColor:"#F97316"}}>{language === 'en' ? 'Service Name' : 'సేవ పేరు'}</th>
      <th style={{backgroundColor:"#F97316"}}>{language === 'en' ? 'Actions' : 'చర్యలు'}</th>
    </tr>
  </thead>
  <tbody>
